...
def visGraph(mode, belief, start_v, bounds, humHist, humPred, humState, robState, robPred, t, bel_frame):
  ...
  for i in range(num_times):
    ...
    for j in range(num_params):
      ...
      ax.set_title('Model Confidence = %0.2f' % (bel_frame))
      #fig, ax = plt.subplots()
      ...
      plt.scatter(humPredX, humPredY, s=5, c='skyblue', label = 'Predictions')
      ...
      s_ego = ax.imshow(red_car, extent=[-0.5/2, 0.5/2, -0.5/4, 0.5/4])
      #s = ax.imshow(FRT_out.T, cmap='bwr', alpha = 0.5 + 0.5*FRT_out.T, origin='lower', extent=(-alpha['x'], alpha['x'], -alpha['y'],alpha['y']), aspect=(alpha['x']/alpha['y']), vmin=-1., vmax=1.)
      ...
      # x = scale*(X-xcent) + xref

      plt.scatter(humHistX, humHistY,marker = '*', s=5, c='b', label = 'History')
      plt.scatter(robPredX, robPredY,marker = '*', s=5, c='red', label = 'Planned Trajectory')
      plt.legend()
      ax.axis('off')
      #plt.scatter(-1,0, s=5)
      #plt.scatter(-0.805,-0.735, s = 1)
      #plt.scatter(-0.805,0.265, s = 1)
      #plt.scatter(0.195,-0.735, s = 1)
      #plt.scatter(1.195,-0.735, s = 1)
      #plt.scatter(0,0, s=5)
      #plt.scatter(-1,0, s=5)

  #fig.savefig(mode + belief +'.png')
  #fig.savefig(mode + belief +'figs.png')  
  fig.savefig("vis" + mode + belief + str(t)+'figs.png', dpi=400)
...
def main(mode, beta_low, gamma):
  ...
  with torch.no_grad():
      for tstep in frames:
          ...
          print(keys)
          print(predictions_mm[tstep][keys[1]])
          print(np.shape(predictions_mm[tstep][keys[1]]))
  ...
  for frame in range(len(frames)):
    ...
    humHistX = humX[:frame]
    humHistY = humY[:frame]
    #humHistThet = humThet[:frame]
    humHist = [humHistX, humHistY]
    robPred = [rob_xTraj, rob_yTraj]
    print('robstate')
    print(robState)

    bel_frame = beta_interp[frame]
    visGraph(mode, 'pred', humStartVel, bounds, humHist, humPred, humState, robState, robPred, frame, bel_frame)
    if collisionBel != [0] and len(collisionBel[0]) != 0:
      ...
      visGraph(mode, 'bel', humStartVel, bounds, humHist, humPred, humState, robBelState, collisionBel, frame, bel_frame)
    if collision != [0] and len(collision[0]) != 0:
      ...
      collision = [colx, coly]
      visGraph(mode, 'nobel', humStartVel, bounds, humHist, humPred, humState, robNobelState, collision, frame, bel_frame)

    #visGraph(mode, '_nobelnew', humStartVel, amin1, amax1, omin1, omax1, amin2, amax2, omin2, omax2, humPred, humState, frame)
    ...
    for t in range(ph+1):   
        ...
        for i in range(len(egoXRange)):
          ...
          if collisionBel == [0]:
            ...
            print('colbel')
            print(querypoint_bel)
            print(collisionBel)
            if len(collisionBel) == 2:
              aRange1 = amax1Bel - amin1Bel
              aRange2 = amax2Bel - amin2Bel
              oRange1 = omax1Bel - omin1Bel
              oRange2 = omax2Bel - omin2Bel
              ...
              #print('------------------------------------------------')
              print('bel')
              visGraph(mode, 'bel', humStartVel, bounds, humHist, humPred, humState, robState, collisionBel, frame, bel_frame)
              #visGraph(mode, '_belnew', humStartVel, amin1Bel, amax1Bel, omin1Bel, omax1Bel, amin2Bel, amax2Bel, omin2Bel, omax2Bel, humPred, t)
          ...
          if collision == [0]:
            ...
            if len(collision) == 2:
              ...
              print(humStartVel, amin1, amax1, omin1, omax1, amin2, amax2, omin2, omax2)
              print('nobel')
              visGraph(mode, 'nobel', humStartVel, bounds, humHist, humPred, humState, robState, collision, frame, bel_frame)
        ...
        if len(collision)==2 and len(collisionBel) == 2:
          if len(collision[0]) == 0 and len(collisionBel[0]) == 0:
