...
def plot_vehicle_nice(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0, ego = False):
    ...
    for node in node_list:
        ...
        if node.type.name == 'VEHICLE':
            ...
            if node == ego_node and ego==False:
                continue
            elif node == ego_node:
                ax.plot(future[:, 0],
                        future[:, 1],
            ...
            if node == ego_node:
                ...
                print(h)
                print('heading state')
                print(node.get(np.array([ts_key]), {'heading': ['°']})[0, 0])
            #r_img = rotate(cars[i % len(cars)], node.get(np.array([ts_key]), {'heading': ['°']})[0, 0] * 180 / np.pi,
            #               reshape=True)
            r_img = rotate(cars[i % len(cars)], h * 180 / np.pi,
                           reshape=True)
...
def plot_vehicle_mm(ax, predictions, dt, max_hl=10, ph=6, map=None, x_min=0, y_min=0, ego = False):
    prediction_dict, histories_dict, futures_dict = prediction_output_to_trajectories(predictions,
                                                                                      dt,
                                                                                      max_hl,
                                                                                      ph,
