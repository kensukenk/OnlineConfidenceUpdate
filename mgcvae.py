...
class MultimodalGenerativeCVAE(object):
    ...
    def p_y_xz(self, mode, x, x_nr_t, y_r, n_s_t0, z_stacked, prediction_horizon,
               num_samples, num_components=1, gmm_mode=False,get_control = False):
        r"""
        ...
        if get_control:
            if mode == ModeKeys.PREDICT:
                ...
                return a_dist, sampled_future
            else:
                return a_dist

        else:
            if mode == ModeKeys.PREDICT:
                ## TODO adjust the covariance matrix
                if gmm_mode:
                    a_sample = a_dist.mode()
                else:
                    a_sample = a_dist.rsample()
    ...
    def predict(self,
        ...
        x, x_nr_t, _, y_r, _, n_s_t0 = self.obtain_encoded_tensors(mode=mode,
                                                                   ...
                                                                   robot=robot,
                                                                   map=map)
        if not z_mode and not all_z_sep:
            full_dist = True
        self.latent.p_dist = self.p_z_x(mode, x)
    ...
    def predict_actions(self,
        ...
        ydist, our_sampled_future = self.p_y_xz(mode, x, x_nr_t, y_r, n_s_t0, z,
                                            prediction_horizon,
                                            num_samples,
                                            num_components,
                                            gmm_mode, get_control = True)
        
        
        ysig = ydist.get_covariance_matrix()
