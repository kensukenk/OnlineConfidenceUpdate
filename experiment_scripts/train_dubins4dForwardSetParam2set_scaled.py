...
p.add_argument('--exact_BC', action='store_true', default=False, required=False, help='Should we impose exact boundary conditions.')
p.add_argument('--time_norm_mode', type=str, default='none', required=False, choices=['none', 'scale_ham', 'scale_PDE'])
p.add_argument('--bound_mode', type=str, default='v2', required=False, choices=['v1', 'v2'])
p.add_argument('--lx_type', type=str, default='max', required=False)
...
p.add_argument('--sample_inside_target', action='store_true', default=False, required=False, help='Whether to sample inside the target set.')
...
if opt.diffModel and opt.exact_BC:
  print('Only one of diffModel and exact_BC modes can be active!')
  raise NotImplementedError

dataset = dataio.ReachabilityDubins4DForwardParam2SetScaled(numpoints=65000, tMin=opt.tMin, tMax=opt.tMax, alphaT=opt.alphaT,
                                            pretrain=opt.pretrain, counter_start=opt.counter_start, counter_end=opt.counter_end,
                                            pretrain_iters=opt.pretrain_iters, num_src_samples=opt.num_src_samples, 
                                            periodic_boundary=opt.periodic_boundary, diffModel=opt.diffModel, exact_BC=opt.exact_BC,
                                            bound_mode=opt.bound_mode, lx_type=opt.lx_type, sample_inside_target=opt.sample_inside_target)
...
# Define the loss
loss_fn = loss_functions.initialize_hji_Car4D(dataset, opt.minWith, opt.time_norm_mode)
...
def val_fn(model, ckpt_dir, epoch):
  ...
  for i in range(num_times):
    ...
    for j in range(num_params):
      # State and time coords
      coords = torch.cat((time_coords, mgrid_coords), dim=1) 

      # Initial velocity coords
      startV_coords = (torch.ones(mgrid_coords.shape[0], 1) * startV[j] - beta['v'])/alpha['v']
      ...
      model_out = model_out.reshape((sidelen[0], sidelen[1], sidelen[2], sidelen[3]))
      ...
      if opt.diffModel:
        ...
        lx = lx.detach().cpu()
        lx = lx.reshape((sidelen[0], sidelen[1], sidelen[2], sidelen[3]))
        model_out = (model_out*dataset.var/dataset.norm_to)
        model_out = model_out + lx
      elif opt.exact_BC:
        lx = dataset.compute_IC(coords[..., 1:])
        lx = lx.detach().cpu()
        lx = lx.reshape((sidelen[0], sidelen[1], sidelen[2], sidelen[3]))
        time = time_coords * 1.0
        time = time.reshape((sidelen[0], sidelen[1], sidelen[2], sidelen[3])).detach().cpu()
        model_out = (model_out*dataset.var/dataset.norm_to) * time
        model_out = model_out + lx
      else:
        model_out = (model_out*dataset.var/dataset.norm_to) + dataset.mean

      # Project along theta and v directions
      model_out = torch.min(model_out, dim=-1, keepdim=False)[0]
      model_out = torch.min(model_out, dim=-1, keepdim=False)[0]
